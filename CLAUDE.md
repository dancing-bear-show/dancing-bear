...
- charts, diagrams, worker, and workflow build their parser via `CLIApp.build_parser()` but skip `run_with_assistant()`, so none of the four support `--agentic`. charts and diagrams dispatch commands directly (never call `CLIApp.run()`) to preserve legacy no-subcommand exit codes/messages — and for diagrams specifically, also to avoid a real crash, since its `--output` dest is a file path that collides with `CLIApp.run()`'s hardcoded output-*format* parsing. worker and workflow use `build_parser()` only for a no-subcommand pre-check and otherwise call `CLIApp.run()` normally.
