...
def apply_plan_file(
    ...
    for op in ops:
        ...
        if action == "move":
            _do_move(op["src"], op["dest"], dry_run=dry_run, writer=writer)
        elif action == "trash":
            _do_trash(op["src"], dry_run=dry_run, writer=writer)
        else:
...
def _do_move(src: str, dest: str, dry_run: bool = False, writer: OutputWriter | None = None) -> None:
    ...
    try:
        ...
        writer.print(f"moved: {src} -> {dest}")
    except Exception as e:  # nosec B110 - log and continue; file-system errors on individual ops should not abort the batch
        writer.print_error(f"moving {src} -> {dest}: {e}")


def _do_trash(src: str, dry_run: bool = False, writer: OutputWriter | None = None) -> None:
    writer = writer or OutputWriter()
    ...
    try:
        shutil.move(src, dest)
        writer.print(f"trashed: {src}")
    except Exception as e:  # nosec B110 - log and continue; file-system errors on individual ops should not abort the batch
        writer.print_error(f"trashing {src}: {e}")
