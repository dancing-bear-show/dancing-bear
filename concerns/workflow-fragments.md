...
### pr-thread-fetch-loses-review-bodies
- **severity**: major
- **check**: `workflows/shared/pr-review-threads.yaml` claims to cover all three GitHub comment surfaces, and its Step 2/3 REST calls are missing `--paginate`. Verify both REST fetches paginate, and verify that a review BODY is decomposed into its individual findings rather than stored as one opaque blob. A consumer that triages the normalised list will otherwise report a clean run while real findings sit unread.
- **triggers**: Any workflow reading PR review feedback via `gh api ".../pulls/<n>/reviews"` or `".../issues/<n>/comments"` without `--paginate`; any stage that treats a review body as a single comment.
- **example**: Measured on PR #391 (2026-09-22): `gh api "repos/OWNER/REPO/pulls/391/reviews"` returns **30** reviews; the same call with `--paginate` returns **72**. So 58% of review bodies were invisible to the fetch. Separately, Copilot nests per-finding detail inside `<details><summary>title</summary> path:line ... </details>` blocks within one review body — the fetch stores that whole blob as a single entry, so its "Previously missed" findings (findings in code that has NOT changed since the last review, which therefore never become review threads) are never triaged. On #391 this hid 14 distinct findings across the run, including a `0d` purge window that would have deleted every done/error record, and a demo handler instruction that raised `NameError` and was therefore retried instead of failing terminally. Every one was found by reading review bodies by hand after `reviewThreads` reported 107/107 resolved. Fix: add `--paginate` to both REST calls, and parse `<details>` blocks into one normalised entry per finding with `source: "review-body-finding"`, carrying its own path/line so the existing by-file fan-out can route it.
