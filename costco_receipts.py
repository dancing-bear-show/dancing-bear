...
def screenshot() -> str:
    subprocess.run(["screencapture", "-x", "-t", "png", "/tmp/screen.png"], capture_output=True)
...
def run_tool(tool_name: str, tool_input: dict) -> str:
    if tool_name == "computer":
        ...
        if action == "screenshot":
            ...
            x, y = tool_input["coordinate"]
            subprocess.run(["cliclick", f"rc:{x},{y}"])
            ...
            subprocess.run(["cliclick", f"t:{tool_input['text']}"])
            ...
            for _ in range(amount):
                subprocess.run(["cliclick", f"s{flag}:{x},{y}"])
