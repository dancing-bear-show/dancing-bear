...
class TestLlmCli(unittest.TestCase):
    def test_help(self):
        ...
        import sys
        root = repo_root()
        proc = subprocess.run([sys.executable, str(bin_path('llm')), '--help'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=str(root))  # nosec S603 - test code with trusted local script
    ...
    def test_check_respects_sla_env(self):
        ...
        proc = subprocess.run([sys.executable, str(bin_path('llm')), 'check'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=str(root), env=env)  # nosec S603 - test code with trusted local script
