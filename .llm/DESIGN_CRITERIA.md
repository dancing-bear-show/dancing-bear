...
- **Known gap (re-measured 2026-08-28):** the packages with zero `SafeProcessor`/`BaseProducer` usage are `resume/`, `slides/`, `sheets/`, `qlty/`, and `charts/`. Of these, `charts/` is a documented deliberate exemption (pure in-memory rendering, no subprocess or network I/O — `src/charts/README.md`), so the genuine gaps are `resume/`, `slides/`, `sheets/`, `qlty/`. An earlier audit listed `telemetry/`, `worker/`, `apple_music/`, `diagrams/`, and `workflow/` as zero; all five now reference the pipeline (`telemetry/parse_transcripts.py`, `telemetry/otel/cli/cost.py`, `worker/handlers.py`, `worker/job_runtime.py`, `apple_music/cli.py`, `diagrams/renderers.py`, `workflow/dispatchers.py`). Re-measure before citing this list: `grep -rl "SafeProcessor\|BaseProducer" src/<package>/ | wc -l`. (`metals/` also had partial adoption, 1 of ~29 files, but was extracted to the separate private `dancing-bear-metals` repo on 2026-08-24 and is no longer part of this codebase.)
...
- Capability checks MUST go through an explicit gate (`"x" in provider.capabilities()`), not `isinstance()` branching on the concrete provider type. `capabilities()` returns `set[str]` (`mail/providers/base.py:130`), so membership is the test — an earlier revision of this criterion wrote it dict-style as `.get("x")`, which raises `AttributeError` on a set.
- **Status:** `capabilities()` currently has no call sites outside its own definition and the two provider overrides. It is a declared contract that is not yet load-bearing; the first real gate should follow the set semantics above.
