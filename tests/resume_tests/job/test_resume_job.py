...
class TestBuildKeywordSpec(unittest.TestCase):
    ...
    def test_build_with_mixed_item_types_and_weight_overrides(self):
        # Integration case: every normalization path (bare string, {"skill":},
        # {"name":}) combined with per-item weight overrides, every keyword
        # tier, category aliases, and synonym string/list forms in one config.
        job_cfg = {
            "keywords": {
        ...
        spec, syn = build_keyword_spec(job_cfg)
