...
def _flow_map() -> str:
    ...
    if _cli_path_exists(["outlook", "add"]) or _cli_path_exists(["outlook", "add-recurring"]):
        ...
        if _cli_path_exists(["outlook", "add"]):
            lines.append("  - One-off: ./bin/calendar-assistant outlook add --subject 'Title' --start 2025-01-01T10:00 --end 2025-01-01T11:00 --calendar 'Your Family'")
        if _cli_path_exists(["outlook", "add-recurring"]):
            lines.append("  - Recurring: ./bin/calendar-assistant outlook add-recurring --subject 'Class' --repeat weekly --byday MO --start-time 17:00 --end-time 17:30 --range-start 2025-10-01 --until 2025-12-15 --calendar 'Your Family'")
    if _cli_path_exists(["outlook", "add-from-config"]) and _cli_path_exists(["outlook", "verify-from-config"]):
        ...
        lines.append("  - Add from config: ./bin/calendar-assistant outlook add-from-config --config schedules/plan.yaml")
        lines.append("  - Verify plan: ./bin/calendar-assistant outlook verify-from-config --config schedules/plan.yaml")
    if _cli_path_exists(["outlook", "update-locations"]) and _cli_path_exists(["outlook", "apply-locations"]):
        lines.append("- Locations")
        lines.append("  - Update from Outlook: ./bin/calendar-assistant outlook update-locations --config schedules/plan.yaml")
        lines.append("  - Apply to Outlook: ./bin/calendar-assistant outlook apply-locations --config schedules/plan.yaml")
    if _cli_path_exists(["outlook", "reminders-off"]):
        lines.append("- Reminders")
        lines.append("  - Turn off: ./bin/calendar-assistant outlook reminders-off --calendar 'Your Family' --from 2025-01-01 --to 2025-12-31 --all-occurrences")
    if _cli_path_exists(["outlook", "dedup"]):
        lines.append("- Deduplicate")
        lines.append("  - Remove dup series: ./bin/calendar-assistant outlook dedup --calendar 'Your Family' --apply")
    if _cli_path_exists(["outlook", "list-one-offs"]):
        lines.append("- List one-offs")
        lines.append("  - List: ./bin/calendar-assistant outlook list-one-offs --calendar 'Your Family' --from 2025-01-01 --to 2025-12-31 --out out/one_offs.yaml")
    if _cli_path_exists(["outlook", "remove-from-config"]):
        lines.append("- Remove from YAML")
        lines.append("  - Delete: ./bin/calendar-assistant outlook remove-from-config --config schedules/plan.yaml --apply")
    if _cli_path_exists(["outlook", "calendar-share"]):
        lines.append("- Share")
        lines.append("  - Share calendar: ./bin/calendar-assistant outlook calendar-share --calendar 'Your Family' --user someone@example.com --role reviewer")
    ...
    if _cli_path_exists(["gmail", "scan-classes"]) or _cli_path_exists(["gmail", "scan-receipts"]) or _cli_path_exists(["gmail", "scan-activerh"]):
        ...
        if _cli_path_exists(["gmail", "scan-classes"]):
            lines.append("  - Classes: ./bin/calendar-assistant gmail scan-classes --days 60 --out out/classes.plan.yaml")
        if _cli_path_exists(["gmail", "scan-receipts"]):
            lines.append("  - Receipts: ./bin/calendar-assistant gmail scan-receipts --days 365 --out out/receipts.plan.yaml")
        if _cli_path_exists(["gmail", "scan-activerh"]):
            lines.append("  - ActiveRH: ./bin/calendar-assistant gmail scan-activerh --days 365 --out out/activerh.plan.yaml")
...
def build_agentic_capsule() -> str:
    ...
    """
    commands = [
        "help: ./bin/calendar-assistant --help",
        "add: ./bin/calendar-assistant outlook add --help",
        "scan: ./bin/calendar-assistant gmail scan-classes --help",
    ]
