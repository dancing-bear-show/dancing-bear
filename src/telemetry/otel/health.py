...
def _print_no_infrastructure_message(data_dir: Path) -> None:
    ...
    print(
        ...
        f"  2. Export these in your shell profile — use port 4327 (gRPC),\n"
        f"     NOT 4328 (that's the HTTP port). 4327 is the default; if you\n"
        f"     set OTEL_GRPC_PORT in your environment before step 1 (the\n"
        f"     compose file reads it, it doesn't set it), use that value\n"
        f"     here instead:\n"
        ...
        f"       export OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4327\n"
