...
- **example**: `template.scannable.yaml` commented "Heading 1 (14pt)" beside `h1_pt: 12`. Related shape: a comment claimed `get_bullet_config()` is consulted only on the bulleted branch when it in fact runs whenever `summary` is a list. Fix: align the comment with real behavior, or delete the setting the code ignores. The `summary` section of `template.dark-friendly.yaml` shows the second branch done well — it omits `item_color` rather than carrying a dead key, and records why: `SummarySectionRenderer` hands that section's config only to `get_bullet_config()`, which reads `plain_bullets` and `bullets.style`/`glyph`, and the bullet writers take no colour argument. Prefer that shape: when a key would be inert, leave it out and comment the omission, so the next editor does not re-add it.

### stale-prose-after-behavior-change
- **severity**: major
- **check**: Verify that when a diff changes a governing value or behavior (a default, a cardinality, a file path, a dependency pin, an environment-variable contract, a parsing rule), every other place stating the old value in prose — a workflow header or summary, a shared contract doc, a security advisory, a module docstring, a "Used by" or consumer list — is updated in the same diff rather than left to contradict the new behavior.
- **triggers**: A diff changing a numeric cardinality, a dependency version pin, an environment-variable contract, or a described architecture, alongside unchanged prose elsewhere (a sibling `.md`, a workflow YAML header, a `SECURITY.md` advisory, a checked-in complexity/target justification file) that still names the old value; comments or docs describing a migration as complete ("no X remains") while a separate consumed artifact still asserts X.
- **example**: `SECURITY.md` asserted pdfplumber was "NOT a viable alternative" because it pinned a vulnerable `pdfminer.six==20251107`, and `pyproject.toml` repeated the claim in a comment — while the same file's `pdf` extra shipped `pdfplumber>=0.11.0`, which #387 then pulled into `[dev]`. Checking the dependency rather than the prose showed the current release, pdfplumber 0.11.10, pins `pdfminer.six==20260107` — the exact version the project already required directly — so the stated rationale no longer held; the same advisory also still named the superseded `20251230` as the installed version in two places. Checking the *whole* range rather than just the latest release is what makes the claim precise: the declared floor `>=0.11.0` spans pins from `20231228` through `20260107`, and 0.11.8 pins the very `20251107` the advisory warned about, so the reassuring fact is true of one release, not of the constraint as written. Fixed by correcting all three statements together. Note the failure mode: the advisory was not wrong when written, and nothing flagged it when the pin moved. When a diff changes a pin or default, grep the repo for the old literal — the prose asserting it will not show up in the diff.
