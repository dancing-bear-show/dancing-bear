...
- Auto-derived agentic schema: mail, calendar, schedule, resume, phone, whatsapp, desk, wifi, maker, apple_music, metals, and slides support `--agentic --agentic-format json` (via `CLIApp.run_with_assistant`) to emit a machine-readable parser schema that never drifts from the real CLI; add `--agentic-compact` to strip low-value fields; add `--agentic-domain <prefix>` to filter to one subcommand group.
