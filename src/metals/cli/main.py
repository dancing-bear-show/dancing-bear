...
@app.argument("--profile", "-p", default="gmail_personal", help="Gmail profile")
@app.argument("--out", "-o", default=None, help="Output CSV path")
def cmd_costs_gmail(args) -> int:
    from ..pipeline import GmailCostsRequest, GmailCostsProcessor, GmailCostsProducer

    out_path = args.out or default_costs_path()
    request = GmailCostsRequest(profile=args.profile, out_path=out_path)
...
def cmd_costs_outlook(args) -> int:
    ...
    out_path = args.out or default_costs_path()
    request = OutlookCostsRequest(profile=args.profile, out_path=out_path)
...
@app.argument("--out-dir", "-o", default=None, help=HELP_OUT_DIR)
def cmd_spot_fetch(args) -> int:
    from ..pipeline import SpotPriceRequest, SpotPriceProcessor, SpotPriceProducer

    out_dir = args.out_dir or default_spot_dir()
    out_path = f"{out_dir}/{args.metal}_spot_cad_daily.csv"
...
@app.argument("--costs", default=None, help="Costs CSV path")
@app.argument("--out", default=None, help="Output CSV path")
def cmd_premium_calc(args) -> int:
    from ..premium import main as premium_main
    argv = ["--costs", args.costs or default_costs_path()]
    if args.out:
...
def cmd_build_summaries(args) -> int:
    ...
    costs = args.costs or default_costs_path()
    out_dir = args.out_dir or default_spot_dir()
    return build_main(["--costs", costs, "--out-dir", out_dir])
...
@app.argument("--gold", default=None, help="Gold summary CSV")
@app.argument("--silver", default=None, help="Silver summary CSV")
def cmd_excel_merge(args) -> int:
    from ..excel_merge import main as merge_main
    out_dir = default_spot_dir()
    gold = args.gold or f"{out_dir}/gold_summary.csv"
    silver = args.silver or f"{out_dir}/silver_summary.csv"
    argv = ["--workbook", args.workbook, "--gold", gold, "--silver", silver]
    if args.out:
        argv.extend(["--out", args.out])
