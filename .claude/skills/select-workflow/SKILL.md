...
| **update-review-concerns** | `workflows/code/update-review-concerns.yaml` | Mine past runs and PR threads for recurring issues the concern library misses. `mode=rereview` instead classifies why findings came back in later review rounds (fix regressions, siblings, collateral doc drift) over the `max_prs` most recent PRs (any state) with >= `min_threads` threads, or one `pr_number` | `mode` (topics\|rereview), `window_days` (topics only), `min_occurrences`, `pr_number`, `max_prs`, `min_threads` |
