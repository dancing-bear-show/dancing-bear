...
### pipe-masked-exit-status-gate
- **severity**: major
- **check**: Verify that a stage running a test/lint/scan command through a pipe (`| tail`, `| grep`, `| echo`) or with `|| true` appended captures the underlying command's exit status, and that any gate field (`all_green`, `ready`, `ready_for_commit`) is derived from recorded per-check results rather than written as a literal `true`.
- **triggers**: Stage descriptions containing a test/lint/scan invocation piped into `tail`, `head`, `grep`, or `echo` with no `PIPESTATUS`/`pipefail` capture beforehand; `|| true` appended to a test command; a `writes_to` JSON block whose gate field (`all_green`, `ready`, `ready_for_commit`, `ready_for_review`) is written as a literal `true` rather than computed from recorded results; prose instructing a stage only to "record" or "note" a failure without also failing the stage or setting the gate false.
- **example**: `workflows/test/coverage-improve.yaml:255` runs `python3 -m unittest discover -s tests/ -t . -p 'test_*.py' -q 2>&1 | tail -5` — a failing baseline suite is hidden because `tail` always exits 0; lines 69 and 312 append `|| true` to `coverage run`, forcing exit 0 unconditionally. `workflows/code/pdffill-domain-build.yaml:921` writes `"all_green": true` as a fixed literal while its sibling fields are `"PASS|FAIL"` placeholders. Fix: capture the real status before the pipe (`status=$?` after redirecting to a log, or `set -o pipefail`), fail the stage when it is non-zero, and compute the gate from every recorded result. `workflows/code/claims-domain-build.yaml:1289-1304` and `workflows/code/qlty-complexity-sweep.yaml:2438-2478` already do this correctly and carry explicit "DERIVED, never a literal" language.
