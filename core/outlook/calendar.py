...
class OutlookCalendarMixin:
    ...
    def _update_permission_role(
        self: OutlookClientBase,
        calendar_id: str,
        permission_id: str,
        role: str
    ) -> Dict[str, Any]:
        """Update an existing calendar permission to a new role."""
        body = {"role": role}
        rr = _requests().patch(
    ...
    def ensure_calendar_permission(
        ...
        for p in perms:
            ...
            if em == target_email:
                ...
                if cur.lower() != target_role:
                    pid = p.get("id")
                    if pid:
                        return self._update_permission_role(calendar_id, pid, role)
                return p
        return self._create_permission(calendar_id, email, role)
...
def _is_word_city(w: str) -> bool:
    """Check if a word is a city name (contains letters, no digits)."""
...
def _extract_canadian_postal_code(toks: List[str]) -> Tuple[Optional[str], List[str]]:
    """Extract Canadian postal code from token list if present."""
    if len(toks) >= 2:
...
def _extract_state_from_tokens(toks: List[str]) -> Optional[str]:
    """Extract 2-letter state code from token list."""
    for t in toks:
        tt = t.strip().strip(",")
...
def _extract_postal_from_tokens(toks: List[str]) -> Optional[str]:
    """Extract postal code from token list (any token with digits)."""
...
def _parse_two_part_address(parts: List[str]) -> Dict[str, Any]:
    """Parse a 2-part address (street, city/state/postal)."""
    tail = parts[1]
    toks = tail.split()

    # Check for Canadian format: state code + postal code
    if len(toks) >= 2 and re.match(r"^[A-Z]{2}$", toks[0]):
        rest = " ".join(toks[1:]).upper()
        if re.match(r"^[A-Z]\d[A-Z]$|^[A-Z]\d[A-Z]\s\d[A-Z]\d$", rest):
            state = toks[0]
            mpc = re.search(r"[A-Z]\d[A-Z]\s?\d[A-Z]\d", rest)
            postal = rest[mpc.start():mpc.end()].replace(" ", " ") if mpc else None

            # Extract city from first part
            cand = parts[0].strip()
            ...
            return {"street": street, "city": city, "state": state, "postal": postal}

    # Default: second part is city
    return {"street": parts[0], "city": parts[1], "state": None, "postal": None}


def _parse_multi_part_address(parts: List[str]) -> Dict[str, Any]:
    """Parse a 3+ part address (street, city, state/postal, country)."""
    city = parts[-2]
    tail = parts[-1]
    toks = tail.split()

    # Try to extract Canadian postal code
    postal, toks = _extract_canadian_postal_code(toks)

    # Extract state code
    state = _extract_state_from_tokens(toks)

    # Extract postal code if not already found
    if not postal:
        postal = _extract_postal_from_tokens(toks)

    # Extract country if 4+ parts
...
def _parse_addr(addr: str) -> Dict[str, Any]:
    ...
    if len(parts) == 2:
        result = _parse_two_part_address(parts)
    else:
        result = _parse_multi_part_address(parts)

    # Build address object with non-None values
    addr_obj: Dict[str, Any] = {}
    if result.get("street"):
        addr_obj["street"] = result["street"]
    if result.get("city"):
        addr_obj["city"] = result["city"]
    if result.get("state"):
        addr_obj["state"] = result["state"]
    if result.get("postal"):
        addr_obj["postalCode"] = result["postal"]
    if result.get("country"):
        addr_obj["countryOrRegion"] = result["country"]
