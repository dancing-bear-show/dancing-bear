...
class TestUpdateLocationsFromConfig(unittest.TestCase):
    def test_updates_yaml_with_address(self):
        ...
        def fake_dump(path, obj):
            captured['path'] = path
            captured['obj'] = obj
        yamlio = types.ModuleType('calendars.yamlio')
        ...
        try:
            ...
            if old_yamlio is None:
                sys.modules.pop('calendars.yamlio', None)
            else:
                sys.modules['calendars.yamlio'] = old_yamlio
            if old_osvc is None:
                sys.modules.pop('calendars.outlook_service', None)
            else:
                sys.modules['calendars.outlook_service'] = old_osvc
