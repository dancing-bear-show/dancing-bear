...
Covers:
  ...
  - CollectorReadProcessor / CollectorProducer  (src/telemetry/otel/collector.py)
...
# ---------------------------------------------------------------------------
# CollectorReadProcessor / CollectorProducer
# ---------------------------------------------------------------------------

class TestCollectorReadProcessorHappyPath(unittest.TestCase):
    """Consumer -> CollectorReadProcessor -> CollectorProducer happy path."""

    def test_happy_path_running_collector_with_files(self) -> None:
        """CollectorReadProcessor reads file metadata and returns a success envelope."""
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            metrics_file.write_text('{"a": 1}\n', encoding="utf-8")

            request = CollectorRequest(data_dir=data_dir, pid=None)
            envelope = CollectorReadProcessor().process(RequestConsumer(request).consume())

        self.assertTrue(envelope.ok())
        result = envelope.unwrap()
        self.assertIsInstance(result, CollectorResult)
        file_names = [fi.name for fi in result.files]
        self.assertIn("metrics.jsonl", file_names)
        metrics_info = next(fi for fi in result.files if fi.name == "metrics.jsonl")
        self.assertGreater(metrics_info.size_bytes, 0)
        self.assertIsNotNone(metrics_info.line_count)
        self.assertEqual(metrics_info.line_count, 1)

    def test_happy_path_no_files_returns_not_found_info(self) -> None:
        """CollectorReadProcessor returns CollectorFileInfo with size_bytes=0 for absent files."""
        from telemetry.otel.collector import CollectorReadProcessor, CollectorRequest

        with tempfile.TemporaryDirectory() as td:
            request = CollectorRequest(data_dir=Path(td), pid=None)
            envelope = CollectorReadProcessor().process(RequestConsumer(request).consume())

        self.assertTrue(envelope.ok())
        result = envelope.unwrap()
        for fi in result.files:
            self.assertEqual(fi.size_bytes, 0)
            self.assertIsNone(fi.line_count)

    def test_happy_path_producer_renders_running_status(self) -> None:
        """CollectorProducer._produce_success writes 'running' when pid is active."""
        from telemetry.otel.collector import CollectorFileInfo, CollectorProducer, CollectorResult

        result = CollectorResult(
            pid=1234,
            running=True,
            files=[CollectorFileInfo(name="metrics.jsonl", size_bytes=512, line_count=5)],
        )
        writer, buf = _make_output_writer()
        producer = CollectorProducer(writer=writer)
        producer._produce_success(result, None)  # noqa: SLF001
    ...
    def test_happy_path_producer_renders_not_running(self) -> None:
        """CollectorProducer renders 'not running' when pid is None/inactive."""
        from telemetry.otel.collector import CollectorFileInfo, CollectorProducer, CollectorResult

        result = CollectorResult(
            pid=None,
            running=False,
            files=[CollectorFileInfo(name="events.jsonl", size_bytes=0, line_count=None)],
        )
        writer, buf = _make_output_writer()
        producer = CollectorProducer(writer=writer)
        producer._produce_success(result, None)  # noqa: SLF001
    ...
    def test_sad_path_processor_returns_error_on_os_error(self) -> None:
        """When file stat raises OSError inside _process_safe, the envelope captures it."""
        from telemetry.otel.collector import CollectorReadProcessor, CollectorRequest

        with tempfile.TemporaryDirectory() as td:
            data_dir = Path(td)
            metrics_file = data_dir / "metrics.jsonl"
            metrics_file.write_text('{"x": 1}\n', encoding="utf-8")

            request = CollectorRequest(data_dir=data_dir, pid=None)
            # Patch Path.stat to raise OSError so _process_safe encounters an error
            with patch("pathlib.Path.stat", side_effect=OSError("stat failed")):
                envelope = CollectorReadProcessor().process(RequestConsumer(request).consume())

        # OSError should surface as error envelope
        self.assertFalse(envelope.ok())
        msg = (envelope.diagnostics or {}).get("message", "")
        self.assertIn("stat failed", msg)

    def test_sad_path_producer_prints_error_from_bad_envelope(self) -> None:
        """CollectorProducer.produce writes error message for a failed envelope to stderr."""
        from telemetry.otel.collector import CollectorProducer

        writer, _ = _make_output_writer()
        producer = CollectorProducer(writer=writer)
        bad_envelope: ResultEnvelope = ResultEnvelope(
            status="error",
            diagnostics={"message": "collector read failed"},
        )
        err_buf = io.StringIO()
        with patch("sys.stderr", err_buf):
            producer.produce(bad_envelope)
        self.assertIn("collector read failed", err_buf.getvalue())
