...
@dataclass
class OutlookCommandCallbacks:
    """Callbacks and helpers for Outlook calendar CLI registration."""

    f_add: Callable
    f_add_recurring: Callable
    f_add_from_config: Callable
    f_verify_from_config: Callable
    f_update_locations: Callable
    f_apply_locations: Callable
    f_locations_enrich: Callable
    f_list_one_offs: Callable
    f_remove_from_config: Callable
    f_dedup: Callable
    f_scan_classes: Callable
    f_schedule_import: Callable
    f_reminders_off: Callable
    f_reminders_set: Callable
    f_calendar_share: Callable
    f_settings_apply: Callable
    f_mail_list: Callable
    add_common_outlook_args: Callable


def register(subparsers, callbacks: OutlookCommandCallbacks):
    ...
    callbacks.add_common_outlook_args(p_add)
    ...
    p_rec.set_defaults(func=callbacks.f_add_recurring)

    # add-from-config
    p_cfg = sub_out.add_parser("add-from-config", help="Add events defined in a YAML file")
    callbacks.add_common_outlook_args(p_cfg)
    ...
    p_verify.set_defaults(func=callbacks.f_verify_from_config)

    # update-locations
    p_update_loc = sub_out.add_parser("update-locations", help="Update YAML event locations from Outlook calendar")
    callbacks.add_common_outlook_args(p_update_loc)
    ...
    p_update_loc.set_defaults(func=callbacks.f_update_locations)

    # apply-locations
    p_apply_loc = sub_out.add_parser("apply-locations", help="Apply locations from YAML to Outlook events")
    callbacks.add_common_outlook_args(p_apply_loc)
    ...
    p_enrich_loc.set_defaults(func=callbacks.f_locations_enrich)

    # list-one-offs
    p_list_one = sub_out.add_parser("list-one-offs", help="List non-recurring (single) events in a calendar window")
    callbacks.add_common_outlook_args(p_list_one)
    ...
    p_sched.set_defaults(func=callbacks.f_schedule_import)

    # reminders-off
    p_rem = sub_out.add_parser("reminders-off", help="Turn off reminders for events in a date window")
    callbacks.add_common_outlook_args(p_rem)
    ...
    p_share.set_defaults(func=callbacks.f_calendar_share)
    ...
    callbacks.add_common_outlook_args(p_set)
    ...
    p_set.set_defaults(func=callbacks.f_settings_apply)

    # mail-list
    p_mail = sub_out.add_parser("mail-list", help="List recent messages (read-only)")
    callbacks.add_common_outlook_args(p_mail)
    ...
    p_mail.set_defaults(func=callbacks.f_mail_list)
