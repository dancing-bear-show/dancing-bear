...
def register(
    ...
    *,
    ...
    add_common_gmail_auth_args(p_g_scan)
    ...
    add_common_gmail_paging_args(p_g_scan, default_days=60, default_pages=5, default_page_size=100)
    ...
    add_common_gmail_paging_args(p_g_rcpts, default_days=365, default_pages=5, default_page_size=100)
    ...
    p_g_rcpts.set_defaults(func=f_scan_receipts)

    # scan-activerh
    p_g_arh = sub_g.add_parser("scan-activerh", help="Generic scan for ActiveRH receipts across programs")
    add_common_gmail_auth_args(p_g_arh)
    p_g_arh.add_argument("--query", help="Raw Gmail query (overrides defaults)")
    add_common_gmail_paging_args(p_g_arh, default_days=365, default_pages=10, default_page_size=100)
    ...
    add_common_gmail_paging_args(p_g_list, default_days=7, default_pages=1, default_page_size=10)
    p_g_list.add_argument("--inbox-only", action="store_true", help=HELP_INBOX_ONLY)
    p_g_list.set_defaults(func=f_mail_list)

    # sweep-top
    p_g_sweep = sub_g.add_parser("sweep-top", help="Find top frequent senders in Inbox over a window")
    add_common_gmail_auth_args(p_g_sweep)
    ...
    p_g_sweep.set_defaults(func=f_sweep_top)
