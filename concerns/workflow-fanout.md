...
- **severity**: PARTIAL — resolved for `outputs/`, `validation/` and `stages/`; STILL A DEFECT for `dispatch/`. Do not read this entry as a clearance: a reviewer skipping the check on the strength of the word "resolved" would miss the one prefix that still doubles. See **status** for the code evidence on each side.
- **status**: PARTIALLY resolved — three prefixes are safe, `dispatch/` is NOT. Verified 2026-09-22 against source. The PROMPT side recognises four: `src/workflow/dispatch.py`'s `_WORKSPACE_SUBDIRS = ("outputs/", "validation/", "stages/", "dispatch/")`, and `_write_paths()` emits `{root}/{f}` for any of them, falling back to `{root}/outputs/{f}` only for a bare filename. But the COPY-BACK side recognises only three: `src/workflow/dispatchers.py:178` and `src/workflow/orchestrator.py:288` both test `("outputs/", "validation/", "stages/")`. So a `writes_to: dispatch/...` entry is told one path in the prompt and copied to `outputs/dispatch/...` — the doubling this concern describes, still live for that one prefix. An earlier revision of this note claimed all four were safe after checking only `dispatch.py`; that was a generalisation from one code path.
- **check**: For `outputs/`, `validation/` and `stages/`, no action — both spellings are correct and `writes_to: ["manifest.json"]` and `["outputs/manifest.json"]` resolve to the same `{workspace}/outputs/manifest.json`. Do not "fix" those by stripping the prefix. For `dispatch/`, the doubling IS real: avoid `writes_to: dispatch/...` until `dispatchers.py` and `orchestrator.py` list that prefix too.
