...
def build_agentic_capsule() -> str:
    ...
    notes_body = "\n".join(
        [
            ...
            "- Body fields go through `gh api -f`, never `-F`. `-F` reads a value starting",
            "  with `@` as a file path and posts its contents; `-f` sends the literal text.",
            "- `threads reply --run-id` re-fetches the full comment chain before posting;",
            "  a comment already carrying the marker yields status `already_replied`.",
