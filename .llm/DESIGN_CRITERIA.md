...
- **Known gap (as of this audit):** `telemetry/`, `worker/`, `apple_music/`, `charts/`, `diagrams/`, `workflow/` have zero `SafeProcessor`/`BaseProducer` usage. (`metals/` also had partial adoption, 1 of ~29 files, but was extracted to the separate private `dancing-bear-metals` repo on 2026-08-24 and is no longer part of this codebase.)
