...
class TestCLI(unittest.TestCase):
    def test_help(self):
        out = subprocess.run(CLI + ["-h"], capture_output=True, text=True)  # noqa: S603
    ...
    def test_extract_and_summarize(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # extract
            r = subprocess.run(CLI + ["extract", "--linkedin", str(li), "--resume", str(rs), "--out", str(data_out)])  # noqa: S603
            ...
            # summarize
            r = subprocess.run(CLI + [  # noqa: S603
...
""".strip(), encoding="utf-8")
    ...
    def test_structure_inference_skippable(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            r = subprocess.run(CLI + ["structure", "--source", str(ref), "--out", str(out)])  # noqa: S603
            self.assertEqual(r.returncode, 0)
