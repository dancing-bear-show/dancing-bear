...
| **update-review-concerns** | `workflows/code/update-review-concerns.yaml` | Mine past runs and PR threads for recurring issues the concern library misses. `mode=rereview` instead classifies why findings came back in later review rounds (fix regressions, siblings, collateral doc drift) | `mode` (topics\|rereview), `window_days`, `min_occurrences`, `pr_number`, `max_prs`, `min_threads` |
