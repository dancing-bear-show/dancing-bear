        ...
        for p in root.rglob("*.json"):
            try:
                if p.stat().st_mtime < cutoff:
                    p.unlink()
                    removed += 1
            except OSError:  # nosec B110 - skip deletion failures
...
def _setup_venv(venv_dir: Path, skip_install: bool) -> bool:
    """Create and install into a venv. Returns True if venv was newly created."""
    ...
    for fname in ('bin/mail', 'bin/mail-assistant'):
        ...
        except OSError:  # nosec B110 - non-critical, safe to skip
            pass
    return venv_created
    ...
    if payload.copy_gmail_example and not cred_path:
        ...
        if ex.exists() and not dest.exists():
            try:
                ...
                cred_path = str(dest)
            except OSError:  # nosec B110 - non-critical setup step
