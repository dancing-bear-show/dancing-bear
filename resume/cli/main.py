...
def cmd_summarize(args: argparse.Namespace) -> int:
    ...
    if out.suffix.lower() in {EXT_YAML, EXT_YML, EXT_JSON}:
...
def _find_structure_in_dirs(
    profile: str, out_dirs: List[Path], extensions: tuple = (EXT_JSON, EXT_YAML, EXT_YML)
) -> Optional[dict]:
...
def _find_structure_in_config(
    profile: str, extensions: tuple = (EXT_JSON, EXT_YAML, EXT_YML)
) -> Optional[dict]:
...
def _load_structure(args: argparse.Namespace) -> Optional[dict]:
    ...
    if args.structure_from:
        ...
        if sf.lower().endswith((EXT_JSON, EXT_YAML, EXT_YML)):
